...
!!! note "Rules и business hooks — не одно и то же"
    ...
    Если вам нужна проверка уровня всей операции, например хватает ли денег, разрешил ли сервер перенос или нужно применить side effects после успеха, используйте transfer-level hooks (`CanStartTransfer`, `CanStartTransferAsync`, `CanCommitTransfer`, `OnTransferSucceeded`), а не `CanDrop`.
    Для быстрого сравнения см. также [Конвейер переноса](transfer-pipeline.md) и [Матрица Drop Policy](drop-policy-matrix.md).
...
flowchart TD
    ...
    CHECK1 -->|Разрешено| PLAN["При валидации цели"]
...
2. **Валидация цели** --- глобальные + инвентарные + слотовые правила цели.
3. **Обмен (swap)** --- правила проверяются в обоих направлениях (A&rarr;B и B&rarr;A). Встречный
   предмет должен иметь право покинуть свой слот и попасть в исходный слот, поэтому обменом нельзя
   положить предмет туда, куда обычный дроп его не пустил бы. Проверка идёт по всем клеткам, которые предмет займёт, а
   не по клетке под курсором и не по одному якорю: клетка, запрещающая предмет, запрещает его и
   тогда, когда предмет приходит на неё хвостом. Это касается и входящего предмета, и каждого
   вытесненного.

### В каком виде правило получает предмет

Правила источника видят предмет в модели исходного инвентаря, правила цели — в модели **целевого**
инвентаря. Если инвентари используют разные модели данных, конвертация выполняется до того, как
сработают правила цели.

Поэтому `CanDrop`, написанный под ваш собственный тип adapter-а, продолжает работать для предметов,
перетащенных из инвентаря с другим хранением, и вызывать converter внутри правила не нужно. См.
[Конвертация предметов](item-conversion-cookbook.md).

Предмет, который нельзя сконвертировать для цели, отклоняется так же, как любым другим правилом, — с
причиной и ещё во время наведения.

Если правила неожиданно вызываются "слишком часто", сначала посмотрите [Логи и отладка](../reference/logs-and-debugging.md) и [Troubleshooting](../reference/troubleshooting.md).
