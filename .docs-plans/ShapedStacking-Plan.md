...
**Открытые вопросы (решить до соответствующего коммита):**
- **C5 (split):** нужен ли сразу split стека сложной формы (отделить N предметов из shaped-стека в новое
  размещение), или на старте достаточно «двигать весь стек целиком»?
- **C7 (унификация):** включать ли «slot как 1×1 grid» в эту работу или отдельной задачей потом?
...
- **Split** (если решим делать сейчас — см. открытый вопрос): отделить N из shaped-стека в **новое** размещение
  (новый футпринт) на N предметов; остаток остаётся в исходном.
...
### C7 — (опционально) Унификация slot-инвентаря как 1×1 grid
- Рассматривать `SlotTopology` как сетку, где каждый предмет занимает 1 ячейку, чтобы shaped/stack логика была
  единой веткой (упрощает планировщик/стор). **Более крупный рефактор** — по решению пользователя либо в этой
  работе, либо отдельной задачей.
...
4. Целевая модель — grid; slot-как-1×1-grid — опционально (C7).
5. Авто-merge на старте; переключаемая опция merge-режима — C8 (для shaped и одноклеточных).
6. Релиз один, после C1..C9; каждый коммит компилируется. Git-операции — на стороне пользователя.
