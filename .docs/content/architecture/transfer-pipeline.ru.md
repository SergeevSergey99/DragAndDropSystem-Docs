...
накрытых клеток, поэтому точно позиционированный предмет не переезжает на чужой якорь.

`SwapDisplacementFallback` решает, что делать, когда зеркальная позиция вытесненному предмету не
подходит. `MirroredOnly` (по умолчанию) отклоняет своп. `VacatedArea` разрешает занять ближайшее
свободное место внутри области, которую предметы обменивают между собой, — за её пределы предмет не
уходит. Разрешение жадное, попредметное, без бэктрекинга.
