...
3. **Обмен (swap)** --- правила проверяются в обоих направлениях (A&rarr;B и B&rarr;A). Встречный
   ...
   положить предмет туда, куда обычный дроп его не пустил бы. Проверка идёт по фактическим слотам, а не по клетке под
   курсором: входящий предмет судится по разрешённому якорю, а каждый вытесненный — по своему
   реальному слоту назначения.
