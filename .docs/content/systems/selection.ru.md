...
Поведение определяется `DropPolicy` целевого инвентаря:

| Политика | Поведение |
|---|---|
| **Atomic** (по умолчанию для batch) | Все предметы должны влезть. Если хоть один не проходит — вся операция отменяется, ничего не переносится |
| **BestEffort** | Переносится всё что влезло, остальное остаётся в источнике |
