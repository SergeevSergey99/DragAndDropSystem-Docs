...
/// <summary>Where a displaced item may go when its mirrored position does not fit.</summary>
...
Оси независимы: своп, вытесняющий **один** предмет, нуждается в фолбэке ровно так же, как
вытесняющий несколько, поэтому это отдельная настройка, а не третье значение первого enum'а.
...
### Фолбэк в освобождаемую область

При `SwapDisplacementFallback.VacatedArea` вытесненный предмет, которому зеркальная позиция не
подошла (вышла за поле, занята посторонним размещением или уже занята другим вытесненным), ищет
свободное место **внутри освобождаемой области** — это след перетаскиваемого предмета плюс, при
свопе внутри одного инвентаря, следы всех вытесненных, минус клетки, которые займёт входящий
предмет. За пределы этой области предмет не выходит никогда: обмен остаётся обменом областями.

Выбирается позиция, ближайшая к зеркальной по манхэттенскому расстоянию; при равенстве — с меньшим
индексом клетки, чтобы результат не зависел от порядка обхода хеш-множества. Позиции, которые
отклоняют правила слота, пропускаются, а не валят весь своп.

Разрешение жадное и попредметное, без бэктрекинга: предмет, обработанный раньше, забирает клетки у
тех, кто идёт следом. Существует раскладка, при которой другое распределение сработало бы, а жадное
— нет.

Пример (`ProcessDrop_MultiSwap_MirroredDestinationOutOfBounds_FallbackDecidesTheOutcome`): сетка
`3×3`, `2×1` стоит на `{0,3}`, `2×2` на `{1,2,4,5}`, тащим `2×2` на `{3,4,6,7}`. Зеркало для `2×1`
даёт клетку выше поля, и при `MirroredOnly` своп отклоняется. При `VacatedArea` `2×1` уходит в
`{2,5}` — ближайшее свободное место в освобождаемой области, а `2×2` встаёт ровно туда, куда его
отпустили.

## Ограничения MVP

- Геометрия вычисляется только через `IInventoryTopology`; grid-specific ветки запрещены.
- Если destination cell отсутствует в топологии источника и фолбэк выключен, swap отклоняется.
...
- Фолбэк не выходит за освобождаемую область и не делает бэктрекинг.
