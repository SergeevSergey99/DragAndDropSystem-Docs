...
3. **Обмен (swap)** --- правила проверяются в обоих направлениях (A&rarr;B и B&rarr;A). Встречный
   ...
   положить предмет туда, куда обычный дроп его не пустил бы. В multi-swap с `AllCoveredPlacements` эта
   проверка отдельно выполняется для каждого вытесненного размещения и его фактического обратного слота.
