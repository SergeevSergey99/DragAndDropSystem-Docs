...
| `Swap` | Пытается выполнить одиночный swap с занятой целью. Оба предмета проверяются правилами того инвентаря, куда попадают, поэтому обменом нельзя обойти правило, которое запретило бы обычный дроп. |
