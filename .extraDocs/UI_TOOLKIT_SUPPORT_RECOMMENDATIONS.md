...
## Долгосрочный ориентир: engine-agnostic core

Это направление, а не запланированный этап. Оно влияет на то, как проектировать contracts в
Этапах 2–5, поэтому зафиксировано здесь.

### Измеренная связанность с Unity

Замер по `Scripts/` (~26 000 строк):

| Слой | LOC | Характер связанности |
|---|---:|---|
| `Core` + `Inventories` + `Rules` | ~10 100 | косметическая |
| `DataBinding` + `Selection` + `Actions` | ~2 400 | смешанная |
| `DragAndDropManager` | ~950 | смешанная |
| `Interaction` + `UI` + `ContextMenu` | ~6 800 | переписывается под каждый backend |
| `Tools` (editor inspectors) | ~3 900 | Unity-only |

В доменной части (`Core/Models`, `Inventories`, `Strategies`, `Rules`) весь контакт с движком
сводится к `Vector2Int` (151 вхождение), атрибутам `SerializeField`/`Tooltip`/`Header` (73),
`Mathf` (13) и `Debug` (10). Настоящей сцепки со сценой — `Transform`, `GameObject`,
`Object` — всего 17 вхождений на ~10 000 строк.

Иначе говоря: JIT transfer, topology, placement, strategies и policy уже алгоритмически
движко-независимы. Их удерживают в Unity типы-значения и атрибуты, а не архитектура.

### Что из этого следует для текущих этапов

Швы, которые нужны UI Toolkit, и швы, которые нужны любому другому движку, — одни и те же:
neutral interaction event, slot identity отдельно от view, slot lifecycle, view geometry,
focus service. UI Toolkit здесь удобен как принудитель: он заставляет доказать отделимость
ядра, не покидая Unity, где есть работающий test baseline.

Практический критерий: если после Этапов 2–5 доменная сборка собирается **без единой ссылки на
`UnityEngine`**, швы выбраны верно. Если не собирается — они выбраны неверно, и это выясняется
дёшево.

Отсюда дополнительное правило к разделу «Нужно»: новые neutral contracts не должны принимать или
возвращать `UnityEngine` типы без необходимости. `Vector2Int` в placement API — приемлемый
компромисс на переходный период, но `Transform`, `GameObject`, `Sprite` и `Component` в
core-контрактах — нет.

### Порядок стоимости другого движка

Оценка для гипотетического Godot-backend, как иллюстрация цены:

- **после честного выполнения Этапов 2–5**: сопоставимо с добавлением ещё одного adapter —
  основное это input и редакторный тулинг заново;
- **напрямую из текущего состояния**: кратно дороже, поскольку `BaseSlot` присутствует в 80
  файлах runtime и втягивает `MonoBehaviour` в каждый контракт.

Отдельные ограничения, которые придётся решать за пределами архитектуры и которые не решаются
выделением сборок:

- сериализация: `ScriptableObject` и prefab-слоты не переносятся, для пользователей это
  разрыв совместимости, а не миграция;
- lifecycle не изоморфен (`Awake`/`OnEnable`/`OnDisable` против аналогов в другом движке) —
  включая эквиваленты ловушки `[ExecuteAlways]`, описанной в Этапе 0;
- поддержка C# и целевых платформ в конкретном движке требует отдельной проверки.

Вывод: engine-agnostic core стоит держать как ориентир при проектировании contracts, но не
превращать в самостоятельную цель до того, как второй backend внутри Unity подтвердит, что
разделение работает.
