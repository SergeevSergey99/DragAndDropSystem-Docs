...
Компонент `FreeFormSlotLayout` --- пример системы размещения для инвентарей, в которых слоты не привязаны к сетке. Предмет появляется в том месте, куда игрок его бросил. Слоты создаются динамически и позиционируются по координатам дропа.
...
## Расширение: предотвращение наложений
...
`FreeFormSlotLayout` --- пример, а не единственный вариант. Вот точки расширения, через которые можно построить любую логику размещения:
