...
### Как работает batch-перенос

При групповом перетаскивании целевой слот (тот, на который бросили) используется только как подсказка. Система сама находит подходящие слоты для каждого предмета.

Batch всегда работает как sequential best-effort. Каждый entry проверяется по
текущему состоянию target: вместившиеся entries фиксируются, не прошедшие остаются
в source. Если policy разрешает partial transfer, из стека переносится только
вместившееся количество.

Swap при batch-переносе не поддерживается — только размещение в свободные или совместимые слоты.

После успешного batch-переноса выделение автоматически снимается.
