...
2. Если у инвентарей разные модели предметов, предмет приводится к модели целевого
   инвентаря.
3. Система проверяет, может ли выбранная цель принять предмет.
4. Если цель подходит, предмет переносится или объединяется со стеком в целевом слоте.
5. Если цель не подходит, `DropPolicySettings` в соответсвии с выьранными опциями решает, что делать дальше: отклонить перенос, найти другое место или попробовать swap.
6. При успехе отправляются события и обновляется DataBinding. При ошибке состояние
   возвращается по снапшоту.
...
| `Swap` | Пытается выполнить одиночный swap с занятой целью. |
...
Если целевой слот занят, а биндинг инвентаря реализует `CanHandleOccupiedSlotDrop` и `ExecuteOccupiedSlotDrop`, то сначала отрабатывает этот обработчик (первым после проверки правил) как чистая проверка,
затем исполняется all-or-nothing. Если `ExecuteOccupiedSlotDrop` возвращает false то перенос отменяется и состояние возвращается. Если true то перенос заканчивается, если переносимые предметы не были обработаны то они исчезнут. Обработчик больше не запускает alternatives или swap.
