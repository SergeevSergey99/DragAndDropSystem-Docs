...
namespace DragAndDropSystem.Rules
{
    ...
    /// При включённом allowItemOverride предмет может переопределить лимит через IStackSizeLimitable
    /// </summary>
    [Serializable]
    public class MaxStackSizeRule : DragRuleBase, IInventoryRule
    {
        ...
        [SerializeField]
        [Tooltip("Разрешить предметам переопределять лимит через IStackSizeLimitable. " +
                 "Если true — предмет с IStackSizeLimitable использует свой лимит вместо _maxStackSize. " +
                 "Если false — _maxStackSize является жёстким потолком для всех предметов.")]
        private bool _allowItemOverride;

        public MaxStackSizeRule() { }

        public MaxStackSizeRule(int maxStackSize, bool allowItemOverride = false)
        {
        ...
        /// <summary>
        /// Эффективный лимит: если allowItemOverride и предмет реализует IStackSizeLimitable —
        /// используется лимит предмета (полное переопределение). Иначе — _maxStackSize.
        /// </summary>
        private int GetEffectiveMaxStackSize(IInventoryItem item)
        {
            if (_allowItemOverride && item is IStackSizeLimitable limitable)
                return limitable.MaxStackSize;
            return _maxStackSize;
        }

        public override RuleResult CanDrop(DragContext context, DragEntry entry)
        {
            int effectiveMax = GetEffectiveMaxStackSize(entry.Stack.Item);
            ...
            if (context.TargetSlot == null || context.IsBatchDrag)
            {
                ...
                if (entry.Stack.Count > effectiveMax)
                {
                    return RuleResult.Failure($"Stack size limit is {effectiveMax}");
                }
            }
            ...
            else if (context.TargetSlot.Stack.CanStack(entry.Stack.Item))
            {
                ...
                if (resultCount > effectiveMax)
                {
                    return RuleResult.Failure($"Stack size limit is {effectiveMax} (would result in {resultCount})");
                }
            }
