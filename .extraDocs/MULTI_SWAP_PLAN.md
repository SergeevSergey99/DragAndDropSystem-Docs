...
попредметное, без бэктрекинга: крупные footprint разрешаются раньше мелких, а предмет,
обработанный раньше, забирает клетки у следующих. Primary placement переносится в начало только после разрешения,
для legacy callback payload, и не влияет на распределение клеток.
